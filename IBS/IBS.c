...
static int do_dscr_pid(int dscr_state, pid_t pid){
	...
	fprintf(stdout, "El proceso %d configura el DSCR %d.\n",pid,dscr_state);
...
static void get_counts(node *aux){
	...
	for(i=0; i < aux->num_fds; i++) {
		...
		if (ret < (ssize_t)sizeof(aux->fds[i].values)) {
			fprintf(stderr,"Error: get_counts.\n");
			if (ret == -1)
		...
		case 0: // Hijo
			...
			switch(node->benchmark) {
					...
					case 4:
						...
						if (fitxer == NULL) {
							printf("Error. No se ha podido abrir el fichero: CPU2006/445.gobmk/data/ref/input/13x13.tst.\n");
							return -1;
						}
						break;
					...
					case 13:
						...
						if (fitxer == NULL) {
							printf("Error. No se ha podido abrir el fichero: CPU2006/416.gamess/data/ref/input/h2ocu2+.gradient.config.\n");
							return -1;
						}
						break;
					...
					case 14:
						...
						if (fitxer == NULL) {
							printf("Error. No se ha podido abrir el fichero: CPU2006/433.milc/data/ref/input/su3imp.in.\n");
							return -1;
						}
						break;
					...
					case 18:
						...
						if (fitxer == NULL) {
							printf("Error. No se ha podido abrir el fichero: CPU2006/437.leslie3d/data/ref/input/leslie3d.in.\n");
							return -1;
						}
						break;
					...
					case 22:
						...
						if (fitxer == NULL) {
							printf("Error. No se ha podido abrir el fichero: working_dir/povray.sal\n");
							return -1;
						}
						break;
				}
		execv(benchmarks[node->benchmark][0], benchmarks[node->benchmark]);
		fprintf(stderr, "Error al lanzar la aplicacion: %d.\n",node->benchmark);
		exit (-2);
...
int main(int argc, char **argv) {
	...
	printf("\nIBS implementation Copyright (C) 2020 Manel Lurbe Sempere <malursem@inf.upv.es>\nThis program comes with ABSOLUTELY NO WARRANTY; for details type `show w'.\nThis is free software, and you are welcome to redistribute it under certain conditions; type `show c' for details.\n");
...
while ((c=getopt(argc, argv,"hgvPODQFA:d:o:S:N:c:b:m:B:")) != -1) {
	...
	if (!options.output_directory){
		fprintf(stderr, "Error: No se ha especificado el directorio de salida.\n");
		return -1;
	}else{
		fichero_out = fopen(options.outputDirectory, "w");
	}
	if (delayMuestreo < 1) {
		delayMuestreo = 1;
	}
	...
	t = time(NULL);
	tm = *localtime(&t);
	fprintf(fichero_out,"FinEjecucion:\t%d-%d-%d\t%d:%d:%d\n", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
  return 0;
}
